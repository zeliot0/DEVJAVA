...
(() => {
  ...
  async function initEditor(id, placeholder = "Commencez à écrire...") {
  ...
  async function setEditorData(id, content) {
    const editor = await initEditor(id);
    if (!content) {
      await editor.clear();
      return;
    }
    ...
    try {
      ...
      if (data && data.blocks) {
        ...
        throw new Error("Not valid blocks");
      }
    } catch (e) {
      // Fallback to simple paragraph if it's plain text
      await editor.render({
  ...
  async function getEditorData(id) {
    if (!editors[id]) return null;
    const output = await editors[id].save();
    return JSON.stringify(output);
  }

  // Expose to window for executions.js
  window.NEXA_EDITOR = { setEditorData, getEditorData, initEditor };
  ...
  // DRAWER (Task)
  // =========================
  async function openDrawer(task = null) {
    ...
    if (!task) {
      ...
      fTitle.value = "";
      await setEditorData("fDescEditor", "");
      fPrio.value = "med";
      fStatus.value = "todo";
      fDue.value = "";
      fCategory.value = "";
    ...
    await setEditorData("fDescEditor", task.description || "");
  ...
  async function getPayload() {
    const desc = await getEditorData("fDescEditor");
    return {
  ...
  function taskCard(task) {
    ...
    el.innerHTML = `
      ...
      ${task.description ? `<div class="task-desc">${escapeHtml(extractPlainText(task.description))}</div>` : ""}
  ...
  // CATEGORY DRAWER
  // =========================
  async function openCatDrawer() {
    catOverlay?.classList.add("open");
    catDrawer?.classList.add("open");
    await setEditorData("catDescEditor", "");
  }

  async function closeCatDrawerFn() {
  ...
  async function createCategory() {
    const descData = await getEditorData("catDescEditor");
    const payload = {
