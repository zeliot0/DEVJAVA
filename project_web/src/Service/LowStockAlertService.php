...
final class LowStockAlertService
{
    ...
    public function notifyIfCrossedThreshold(Produit $produit, ?int $previousStock = null): ?bool
    {
        ...
        $to = $this->resolveRecipient($produit);
    ...
    private function resolveRecipient(Produit $produit): string
    {
        $productEmail = trim((string) $produit->getEmailP());
        if ($productEmail !== '') {
            return $productEmail;
        }
        ...
        return $this->resolveDefaultRecipient();
    }

    private function resolveDefaultSender(): string
    {
        $fromValue = trim((string) $this->mailerFrom);
        if ($fromValue !== '') {
            return $fromValue;
        }
