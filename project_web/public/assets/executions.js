...
(() => {
  ...
  function openDrawer(taskId, taskTitle, mode = "full") {
  ...
  function render() {
    ...
    filtered.forEach(e => {
      ...
      const actionsHtml = (currentMode === "view")
        ...
        : `
          <div class="exe-actions">
            ...
            <button class="mini-btn danger" data-act="del" data-id="${e.id}" title="Supprimer">
      ...
      item.innerHTML = `
        <div class="exe-left">
          <div class="exe-status-dot ${e.status}"></div>
          <div class="exe-info">
            <div class="exe-title-text">${escapeHtml(e.title || "")}</div>
            <div class="exe-desc-text">${escapeHtml(e.desc || "")}</div>
          </div>
        </div>
        ${actionsHtml}
      `;
